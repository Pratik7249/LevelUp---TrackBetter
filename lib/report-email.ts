...
export function buildReportHtml(state: TrackerState, kind: string) {
  ...
  const messDays = Object.entries(state.messCompletions).filter(([date, done]) => done && date >= reportRange.from && date <= reportRange.to).length;
